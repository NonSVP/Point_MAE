...
def build_opti_sche(base_model, config):
    ...
    if opti_config.type == 'AdamW':
        def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
            decay = []
            no_decay = []
            # FIX: Handle both Single GPU and Distributed (DataParallel)
            # Use model.named_parameters() directly instead of model.module
    ...
    # FIX: Support both 'Cosine' (from your YAML) and 'CosLR'
    if sche_config.type == 'LambdaLR':
        scheduler = build_lambda_sche(optimizer, sche_config.kwargs)
    elif sche_config.type == 'CosLR' or sche_config.type == 'Cosine':
